...
CUSTOMER_ADDRESS_START_ID = 200412
...
def generate_restaurant_data(restaurant_start_id, restaurant_end_id, location_df):
    ...
    for _, location_row in active_locations.iterrows():
        ...
        for _ in range(num_restaurants_for_location):
            ...
            created_date = fake.date_time_between(start_date=pd.to_datetime(location_created_date), end_date=end_date)
...
def generate_menu_data(menu_start_id, menu_end_id, restaurant_df):
    ...
    for restaurant_id in restaurant_df['RestaurantID']:
        ...
        for _ in range(num_items):
            ...
            modified_date = fake.date_time_between(start_date=created_date, end_date=end_date)
...
def generate_customer_address_data(customer_address_start_id, customer_address_end_id, customer_df, location_df):
    ...
    for customer_id in customer_df['CustomerID']:
        ...
        for idx, location_row in enumerate(customer_locations.iterrows()):
            ...
            created_date = fake.date_time_between(start_date=pd.to_datetime(customer_created_date), end_date=end_date)
            modified_date = fake.date_time_between(start_date=created_date, end_date='now') if random.random() > 0.3 else None
...
def generate_login_audit_data(customer_login_audit_start_id, customer_login_audit_end_id, customer_df):
    ...
    for i in range(customer_login_audit_start_id, customer_login_audit_end_id):
        ...
        start_date = datetime(2020, 8, 1, 0, 0, 0)
        end_date = datetime(2020, 12, 31, 23, 59, 59)
        last_login = fake.date_time_between(start_date=pd.to_datetime(customer_created_date), end_date=end_date)
...
location_df = pd.read_csv('data5/location.csv')

restaurant_df = pd.read_csv('data5/restaurant.csv')
...
restaurant_df = restaurant_df[restaurant_df['CreatedDate'].dt.year.isin([2020])]
print('filtered customer data', restaurant_df)

menu_df = pd.read_csv('data5/menu_items.csv') ## menu_items.csv
...
menu_df = menu_df[menu_df['CreatedDate'].dt.year.isin([2020])]
print('filtered customer data', menu_df)


customer_df = pd.read_csv('data5/customer.csv')

customer_df['CreatedDate'] = pd.to_datetime(customer_df['CreatedDate'])
customer_df = customer_df[customer_df['CreatedDate'].dt.year.isin([2020])]
print('filtered customer data', customer_df)

address_df = pd.read_csv('data5/customer_address.csv')
address_df['CreatedDate'] = pd.to_datetime(address_df['CreatedDate'])
address_df = address_df[address_df['CreatedDate'].dt.year.isin([2020])]
print('filtered customer data', address_df)

# login_audit_df = pd.read_csv('data5/login_audit.csv')

# delivery_agent_df = pd.read_csv('data1/delivery_agent.csv')
delivery_agent_df = pd.read_json('data5/delivery_agent.json')
delivery_agent_df['CreatedDate'] = pd.to_datetime(delivery_agent_df['CreatedDate'])
delivery_agent_df = delivery_agent_df[delivery_agent_df['CreatedDate'].dt.year.isin([2020])]
...
order_items_df_old1 = pd.read_csv('data5/order_items.csv')

order_items_df_merged = pd.concat([order_items_df_old1, order_items_df_old], ignore_index=True)
order_items_df_merged = pd.concat([order_items_df, order_items_df_merged], ignore_index=True)
...
# location_df.to_csv('data5/location.csv',index=False)
# customer_df.to_csv('data5/customer.csv', index=False)
# address_df.to_csv('data5/customer_address.csv', index=False)
# delivery_agent_df.to_csv('data5/delivery_agent.csv', index=False)

# delivery_agent_df['CreatedDate'] = delivery_agent_df['CreatedDate'].dt.strftime('%Y-%m-%d %H:%M:%S')
# delivery_agent_df['ModifiedDate'] = delivery_agent_df['ModifiedDate'].dt.strftime('%Y-%m-%d %H:%M:%S')
# delivery_agent_df.to_json('data5/delivery_agent.json',orient='records', lines=False, indent=4)

restaurant_df.to_csv('data6/restaurant.csv', index=False)
menu_df.to_csv('data6/menu_items.csv', index=False)
# login_audit_df.to_csv('data6/login_audit.csv', index=False)
order_df.to_csv('data6/orders.csv', index=False)
order_items_df.to_csv('data6/order_items.csv', index=False)
delivery_df.to_csv('data6/delivery.csv', index=False)
