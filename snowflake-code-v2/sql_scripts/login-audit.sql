...
-- First, let's check the data from the external stage
select $1 as LoginID,
$2 as CustomerID, 
$3 as LoginType, 
$4 as DeviceInterface,
$5 as MobileDeviceName,
$6 as WebInterface,
$7 as LastLogin      
from @stage_sch.aws_s3_stage/login_audit.csv
(file_format=> SWIGGY_DB.STAGE_SCH.CSV_FILE_FORMAT);

-- STAGING AREA CODE
-- Create stage table with all columns as text for raw data loading
...
from (
    ...
    from @SWIGGY_DB.STAGE_SCH.AWS_S3_STAGE/2025/4/9/login_audit.csv
)
file_format=(format_name= 'swiggy_db.STAGE_SCH.CSV_FILE_FORMAT');

-- Check if data loaded into stream
select * from swiggy_db.stage_sch.login_audit_stm;

-- CLEAN LAYER
use schema swiggy_db.clean_sch;
...
-- Merge data from stage stream to clean table
...
WHEN MATCHED AND (
    ...
    target.device_interface != source.device_interface OR
    target.mobile_device_name != source.mobile_device_name OR
    target.web_interface != source.web_interface OR
    target.last_login != source.last_login OR
...
-- Check if data loaded into clean table
select * from swiggy_db.clean_sch.login_audit;

-- CONSUMPTION LAYER
use schema swiggy_db.consumption_sch;
...
create or replace table swiggy_db.consumption_sch.login_audit_fact (
    ...
    -- ETL metadata
    dw_created_dt timestamp_tz(9) not null default current_timestamp(),
    dw_modified_dt timestamp_tz(9)
)
comment = 'Fact table for login audit events with time dimension attributes';

-- -- Create an index for faster queries on customer_id
-- CREATE INDEX idx_login_audit_customer_id ON swiggy_db.consumption_sch.login_audit_fact(customer_id);
-- -- Create an index for faster queries on login date range
-- CREATE INDEX idx_login_audit_last_login ON swiggy_db.consumption_sch.login_audit_fact(last_login);

-- Merge data from clean stream to fact table
MERGE INTO swiggy_db.consumption_sch.login_audit_fact AS target
...
WHEN MATCHED AND source.METADATA$ACTION = 'INSERT' AND source.METADATA$ISUPDATE = 'TRUE' THEN
    UPDATE SET 
        ...
        target.dw_modified_dt = current_timestamp()
WHEN NOT MATCHED THEN
    INSERT (
        login_hk,
        login_id,
...
-- Check if data loaded into fact table
select * from swiggy_db.consumption_sch.login_audit_fact
limit 100;

-- Create a view to simplify consumption for BI tools
-- create or replace view swiggy_db.consumption_sch.vw_login_activity as
-- select 
--     l.login_id,
--     l.customer_id,
--     -- c.customer_name,  -- Assuming customer dimension exists with this column
--     l.login_type,
--     l.device_interface,
--     l.mobile_device_name,
--     l.web_interface,
--     l.last_login,
--     l.device_category,
--     l.is_mobile,
...
--     -- Time-based metrics
--     case 
--         when l.login_hour between 6 and 11 then 'Morning'
--         when l.login_hour between 12 and 17 then 'Afternoon'
--         when l.login_hour between 18 and 21 then 'Evening'
--         else 'Night'
--     end as time_of_day,
    
--     case 
--         when l.login_day in ('Saturday', 'Sunday') then 'Weekend'
--         else 'Weekday'
--     end as day_type
-- from 
--     swiggy_db.consumption_sch.login_audit_fact l
-- left join 
--     swiggy_db.consumption_sch.customer_dim c  -- Join to customer dimension if exists
--     on l.customer_id = c.customer_id;


-- select * from vw_login_activity;
drop view vw_login_activity;
    

-- Code to load incremental/delta data (similar to the location example)
-- For future delta loads, uncomment and use this code:

/*
copy into swiggy_db.stage_sch.login_audit (
    loginid, customerid, logintype, deviceinterface, 
    mobiledevicename, webinterface, lastlogin, 
...
from (
    select 
        ...
        t.$3::text as logintype,
        t.$4::text as deviceinterface,
        t.$5::text as mobiledevicename,
        t.$6::text as webinterface,
        t.$7::text as lastlogin,
        metadata$filename as _stg_file_name,
        metadata$file_last_modified as _stg_file_load_ts,
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_sch.aws_s3_stage/delta/login_audit/delta-login-audit.csv t
)
