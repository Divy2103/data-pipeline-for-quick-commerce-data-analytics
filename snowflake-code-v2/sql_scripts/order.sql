...
copy into stage_sch.orders (order_id, customer_id, restaurant_id, order_date, total_amount, discount_amount,                                delivery_charges, final_amount, status, payment_method, is_first_order, coupon_code, 
...
from (
    ...
    from @stage_sch.aws_s3_stage/2025/4/9/orders.csv t
)
...
WHEN MATCHED THEN
    ...
    UPDATE SET
        TOTAL_AMOUNT = TRY_TO_DECIMAL(source.total_amount),
        STATUS = source.STATUS,
        PAYMENT_METHOD = source.payment_method,
        MODIFIED_DT = TRY_TO_TIMESTAMP_TZ(source.modified_date),
...
WHEN NOT MATCHED THEN
    ...
    VALUES (
        ...
        TRY_TO_TIMESTAMP(source.order_date),
...
CREATE OR REPLACE TABLE swiggy_db.consumption_sch.orders_fact (
    ...
    eff_start_date timestamp_tz(9) not null,                    -- Effective start date
    eff_end_date timestamp_tz(9),                                                -- Effective end date (NULL for active record)
    is_current BOOLEAN DEFAULT TRUE                                        -- Flag for current record
)
COMMENT = 'Dim table for orders entity with SCD2 support.';

-- Now implement the MERGE statement to populate the orders_fact table
MERGE INTO consumption_sch.orders_fact AS target
...
-- Add a select to verify the results after processing
