...
CREATE OR REPLACE TABLE swiggy_db.clean_sch.menu (
    ...
    Created_date TIMESTAMP_NTZ,               -- Date when the record was created
    Modified_date TIMESTAMP_NTZ,              -- Date when the record was last modified
...
truncate table swiggy_db.stage_sch.menu;
truncate table swiggy_db.clean_sch.menu;
truncate table swiggy_db.consumption_sch.menu_dim;
...
BEGIN
    ...
    SELECT 
        ...
        TRY_CAST(created_date AS TIMESTAMP_NTZ) AS Created_date,  -- Renamed column
        TRY_CAST(modified_date AS TIMESTAMP_NTZ) AS Modified_date, -- Renamed column
        _stg_file_name,
        _stg_file_load_ts,
        _stg_file_md5,
        _copy_data_ts
...
CALL SWIGGY_DB.COMMON.MENU_MAIN_PROCEDURE('@STAGE_SCH.AWS_S3_STAGE/2025/4/17/menu_items.csv');
