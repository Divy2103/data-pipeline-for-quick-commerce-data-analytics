...
from (
    ...
    from @stage_sch.aws_s3_stage/2025/4/9/delivery.csv t
)
...
WHEN MATCHED THEN
    ...
    UPDATE SET
        ...
        delivered_time = source.delivered_time,
        customer_address_id_fk = TO_NUMBER(source.address_id),
        delivery_date = TO_TIMESTAMP(source.delivery_date),
        created_date = TO_TIMESTAMP(source.created_date),
        modified_date = TO_TIMESTAMP(source.modified_date),
...
WHEN NOT MATCHED THEN
    ...
    VALUES (
        ...
        TO_TIMESTAMP(source.delivery_date),
        TO_TIMESTAMP(source.created_date),
        TO_TIMESTAMP(source.modified_date),
...
-- Now implement the MERGE statement to populate the delivery_dim table
...
ON 
    ...
    target.estimated_time = source.estimated_time AND
    target.delivered_time = source.delivered_time AND
    target.customer_address_id_fk = source.customer_address_id_fk AND
